...
class LayerAlignBenchmark(object):
    def __init__(
            self,
            network,
            teacher_network,
        ...
        self.teacher_net = teacher_network.to(self.device)
    ...
    def _init_model(self, network):
        ...
        if self.config_manager.use_init_model:
            ...
            model_checkpoint_path = os.path.join(
                ...
                self.config_manager.init_model_path
            )

            ckpt_step = self.model_manager.restore_model_and_optimizer(
                self.net, self.optimizer, model_checkpoint_path
            )
            LOG.info(f"Loading checkpoint from {model_checkpoint_path}, ckpt_step: {ckpt_step}")
    ...
    def do_train_step(self, step_context: StepContext, _input_datas):
        ...
        if self.config_manager.use_mix_precision:
            ...
            with torch.no_grad():
                teacher_rst_list, teacher_align_layers = self.teacher_net(data_list)

            rst_list, align_layers = self.net_wrapper(data_list)
            total_loss, info_list, acc_list = self.net.compute_loss(data_list, rst_list, align_layers, teacher_rst_list, teacher_align_layers)
            results["total_loss"] = total_loss.item()
            total_loss.backward()
            if (
                    self.distributed_backend == "horovod" and self.node.has_hvd
