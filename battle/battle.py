...
from hok.common.camp import GameMode
...
flags.DEFINE_integer("max_iter_num", -1, "max iter num for camp iterator")
flags.DEFINE_integer("max_frame_num", -1, "max_frame_num")
...
GamecoreClientConfig = NamedTuple(
    "GamecoreClientConfig",
    [("server_addr", str), ("req_timeout", int), ("max_frame_num", int)],
)
...
class Battle:
    ...
    def run(
        ...
        try:
            ...
            LOG.info("No camp_hero_list available, return")
            return
        ...
        while (
            self.task.max_episode <= 0 or episode_cnt < self.task.max_episode
        ) and not self.stop:
            ...
            if (
                ...
                try:
                    ...
                    LOG.info("No camp_hero_list available, break")
                    break
    ...
    def print_task_result(self):
        ...
        if total_num == 0:
            ...
            LOG.info(
                "Win Rate: TaskID({task_id}) Camp1({win_0}/{total_num} {win_rate_0:.2f}) Camp2({win_1}/{total_num} {win_rate_1:.2f})",
                task_id=self.task.task_id,
                win_0=win_num[0],
                total_num=total_num,
...
def thread_safe_iterator2(iterator, max_iter_num=-1):
    lock = threading.Lock()
    cnt = 0
    while True:
        try:
            with lock:
                cnt += 1
                if max_iter_num > 0 and cnt > max_iter_num:
                    return
                value = next(iterator)
        except StopIteration:
            return
...
def main(_):
    ...
    camp_iter = thread_safe_iterator2(camp_iter, FLAGS.max_iter_num)

    gamecore_config = GamecoreClientConfig(
        server_addr=FLAGS.gamecore_server,
        req_timeout=FLAGS.gamecore_req_timeout,
