...
class Model(nn.Module):
    ...
    def forward(self, data_list, data_list_with_label=None, inference=False):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            # hero_target_list.append(organ_emy_hidden_pool.unsqueeze(1)) # 1 emy organ
            hero_target_list.append(organ_emy_hidden_pool.reshape((-1, 1, 32))) # 1 emy organ
            # hero_target_list.append(torch.zeros((monster_list.shape[0], 1, 32))) # 1 emy organ
            hero_target_list.insert(0, torch.ones_like(hero_target_list[2], dtype=torch.float32) * 0)
