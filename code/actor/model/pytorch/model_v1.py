...
class Model(nn.Module):
    ...
    def forward(self, data_list, data_list_with_label=None, inference=False):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            if not inference:
                hero_feature = hero_data[0]#(512,4586)
            else:
                hero_feature = hero_data

            img_fet_dim = np.prod(self.hero_seri_vec_split_shape[hero_index][0])#1734
            vec_fet_dim = np.prod(self.hero_seri_vec_split_shape[hero_index][1])#2852
            feature_img, feature_vec = hero_feature.split([img_fet_dim, vec_fet_dim], dim=1)#(512,1734) (512,2852)

            feature_img_shape = list(self.hero_seri_vec_split_shape[0][0])#[-1, 6, 17, 17]
            feature_img_shape.insert(0, -1) # (bs, c, h, w)
            feature_vec_shape = list(self.hero_seri_vec_split_shape[0][1])#[-1, 2852]
            feature_vec_shape.insert(0, -1)

            _feature_img = feature_img.reshape(feature_img_shape)#(512, 6, 17, 17)
            _feature_vec = feature_vec.reshape(feature_vec_shape)#(512, 2852)

            conv_result = self.conv_layers(_feature_img)#(512,6,17,17) -> (512,12,8,8)
            flatten_conv_result = conv_result.flatten(start_dim=1)#(512,12,8,8) -> (512,768)

            split_feature_vec = _feature_vec.split([
                self.all_hero_feature_dim,    #1550
                self.all_soldier_feature_dim, #500
                self.all_organ_feature_dim,   #174
                self.all_monster_feature_dim, #560
                self.global_feature_dim       #68
            ], dim=1)

            hero_vec_list = split_feature_vec[0].split([
                int(np.sum(DimConfig.DIM_OF_HERO_FRD)), #753
                int(np.sum(DimConfig.DIM_OF_HERO_EMY)), #753
                int(np.sum(DimConfig.DIM_OF_HERO_MAIN)) #44
            ], dim=1)
            hero_frd = hero_vec_list[0].split(DimConfig.DIM_OF_HERO_FRD, dim=1)#[(512,251), (512,251), (512,251)]
            _hero_frd = torch.stack(hero_frd, dim=1)#(512, 3, 251)
            hero_emy = hero_vec_list[1].split(DimConfig.DIM_OF_HERO_EMY, dim=1)#[(512,251), (512,251), (512,251)]
            _hero_emy = torch.stack(hero_emy, dim=1)#(512, 3, 251)
            hero_main = hero_vec_list[2].split(DimConfig.DIM_OF_HERO_MAIN, dim=1)[0]#(512,44)
            ...
            _organ_1_3 = torch.stack(organ_1_3, dim=1)#(512, 3, 29)
            organ_4_6 = organ_vec_list[1].split(DimConfig.DIM_OF_ORGAN_4_6, dim=1)
            _organ_4_6 = torch.stack(organ_4_6, dim=1)#(512, 3, 29)

            monster_vec_list = split_feature_vec[3]#(512, 560)
            monster_1_20 = monster_vec_list.split(DimConfig.DIM_OF_MONSTER_1_20, dim=1)
            _monster_1_20 = torch.stack(monster_1_20, dim=1)#(512,20,28)

            global_info = split_feature_vec[4]#(512,68)

            ''' real computations
            '''
            # hero_share
            hero_frd_mlp_out = self.hero_mlp(_hero_frd)#(512,3,251)->(512,3,128)
            hero_frd_fc_out = self.hero_frd_fc(hero_frd_mlp_out)#(512,3,128)->(512,3,64)
            pool_frd_hero, _ = hero_frd_fc_out.max(dim=1)#(512,64)

            hero_emy_mlp_out = self.hero_mlp(_hero_emy)#(512,3,251)->(512,3,128)
            hero_emy_fc_out = self.hero_frd_fc(hero_emy_mlp_out)#(512,3,128)->(512,3,64)
            pool_emy_hero, _ = hero_emy_fc_out.max(dim=1)#(512,64)

            # soldier_share
            soldier_frd_mlp_out = self.soldier_mlp(_soldier_1_10)#(512,10,25)->(512,10,64)
            soldier_frd_fc_out = self.soldier_frd_fc(soldier_frd_mlp_out)#(512,10,64)->(512,10,32)
            # torch.max returns both the max old_values and the argmax indices
            pool_frd_soldier, _ = soldier_frd_fc_out.max(dim=1)#(512,32)

            soldier_emy_mlp_out = self.soldier_mlp(_soldier_11_20)#(512,10,25)->(512,10,64)
            soldier_emy_fc_out = self.soldier_emy_fc(soldier_emy_mlp_out)#(512,10,64)->(512,10,32)
            pool_emy_soldier, _ = soldier_emy_fc_out.max(dim=1)#(512,32)

            # monster_share
            monster_mlp_out = self.monster_mlp(_monster_1_20)#(512,20,28)->(512,20,32)
            pool_monster, _ = monster_mlp_out.max(dim=1)#(512,32)

            # organ_share
            organ_frd_mlp_out = self.organ_mlp(_organ_1_3)#(512,3,29)->(512,3,64)
            organ_frd_fc_out = self.organ_frd_fc(organ_frd_mlp_out)#(512,3,64)->(512,3,32)
            pool_frd_organ, _ = organ_frd_fc_out.max(dim=1)#(512,32)

            organ_emy_mlp_out = self.organ_mlp(_organ_4_6)#(512,3,29)->(512,3,64)
            organ_emy_fc_out = self.organ_emy_fc(organ_emy_mlp_out)#(512,3,64)->(512,3,32)
            pool_emy_organ, _ = organ_emy_fc_out.max(dim=1)#(512,32)

            # hero_main
            main_hero = self.hero_main_mlp(hero_main)#(512,44)->(512,32)
            ...
            concat_result = torch.cat(
                [flatten_conv_result, pool_frd_soldier, pool_emy_soldier, pool_monster,
                 ...
                 pool_emy_hero, global_info], dim=1) #(512,1156)

            fc_concat_result = self.concat_mlp(concat_result)#(512,1156)->(512,256)
            hero_public_result = fc_concat_result.split([64, 192], dim=1)#[(512,64),(512,192)]
            # shared encoding      #[(512,64),(512,64),(512,64)] 三个英雄的数据合并成一个list
            hero_public_first_result_list.append(hero_public_result[0])
            # individual encoding  #[(512,192),(512,192),(512,192)] 三个英雄的数据合并成一个list
            hero_public_second_result_list.append(hero_public_result[1])

        pool_hero_public, _ = torch.stack(hero_public_first_result_list, dim=1).max(dim=1)#(512,64)

        for hero_index in range(self.hero_num):
            hero_result_list = []
            fc_public_result = torch.cat([pool_hero_public, hero_public_second_result_list[hero_index]], dim=1)#(512,256)
