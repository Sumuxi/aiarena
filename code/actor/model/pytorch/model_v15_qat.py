...
class Model(nn.Module):
    def __init__(self, ModelConfig):
        ...
        self.soldier_frd_mlp = nn.Linear(128, 128)
        self.soldier_emy_mlp = nn.Linear(128, 128)
        self.organ_share_mlp = nn.Sequential(
            nn.Linear(29, 128),
            nn.ReLU(),
        )
        self.organ_frd_mlp = nn.Linear(128, 128)
        self.organ_emy_mlp = nn.Linear(128, 128)
        ...
        # 使用 nn.ModuleList 来包裹多个 QuantStub 和 DeQuantStub
        # 量化
        # self.quant = nn.ModuleList([QuantStub()] * 3)
        # 反量化
        # self.dequant = nn.ModuleList([DeQuantStub()] * 3)

        # 部分算子需用下面的模块替换
        self.q_concat1 = nn.quantized.FloatFunctional()
        self.q_concat2 = nn.quantized.FloatFunctional()
        self.q_concat3 = nn.quantized.FloatFunctional()
        self.q_concat4 = nn.quantized.FloatFunctional()
        self.q_concat5 = nn.quantized.FloatFunctional()
        self.q_concat6 = nn.quantized.FloatFunctional()
        self.q_concat7 = nn.quantized.FloatFunctional()
    ...
    def forward(self, data_list, data_list_with_label=None, inference=False):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            # quant
            # hero_feature = self.quant[hero_index](hero_feature)
            ...
            all_hero_target_list.append(self.q_concat1.cat(hero_target_list, dim=1))

            concat_hidden = self.q_concat2.cat(
            ...
            hero_public_first_result_list.append(concat_hidden_split[0].reshape(-1, 1, 128))
            hero_public_second_result_list.append(concat_hidden_split[1])

        pool_hero_public, _ = self.q_concat3.cat(hero_public_first_result_list, dim=1).max(dim=1)

        for hero_index in range(self.hero_num):
            hero_result_list = []
            fc_public_result = self.q_concat4.cat([pool_hero_public, hero_public_second_result_list[hero_index]], dim=1)
            # 4 action head
        ...
        self.probs_h0 = torch.flatten(self.q_concat5.cat(all_hero_predict_result_list[0], 1), start_dim=1)
        self.probs_h1 = torch.flatten(self.q_concat6.cat(all_hero_predict_result_list[1], 1), start_dim=1)
        self.probs_h2 = torch.flatten(self.q_concat7.cat(all_hero_predict_result_list[2], 1), start_dim=1)

        rst_list = [self.probs_h0, self.probs_h1, self.probs_h2]
