...
from model.model_config import ModelConfig as Config
...
class Model(nn.Module):
    def __init__(self):
        ...
        # 部分算子需用下面的模块替换
        self.q_concat = nn.quantized.FloatFunctional()
    ...
    def forward(self, data_list, data_list_with_label=None, inference=False):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            all_hero_target_list.append(self.q_concat.cat(hero_target_list, dim=1))

            concat_hidden = self.q_concat.cat(
        ...
        for hero_index in range(self.hero_num):
            hero_result_list = []
            fc_public_result = self.q_concat.cat([pool_hero_public, hero_public_second_result_list[hero_index]], dim=1)
            # 4 action head
            ...
            # value head
            hero_result_list.append(torch.zeros((hero_result_list[-1].shape[0], 1)))
        ...
        self.probs_h0 = torch.flatten(self.q_concat.cat(all_hero_predict_result_list[0], 1), start_dim=1)
        self.probs_h1 = torch.flatten(self.q_concat.cat(all_hero_predict_result_list[1], 1), start_dim=1)
        self.probs_h2 = torch.flatten(self.q_concat.cat(all_hero_predict_result_list[2], 1), start_dim=1)

        self.probs_h0 = self.dequant[0](self.probs_h0)
        self.probs_h1 = self.dequant[1](self.probs_h1)
        self.probs_h2 = self.dequant[2](self.probs_h2)
