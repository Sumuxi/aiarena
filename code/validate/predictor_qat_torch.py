...
from model.model_v15_qat import Model


def fuse_model(module_in):
    module_in.eval()
    module_in.conv_layers = torch.quantization.fuse_modules(module_in.conv_layers, ['0', '1'])
    module_in.train()
    return module_in
    ...
    def reset(self, model_path=None):
        ...
        if model_path:
            ...
            checkpoint = torch.load(
                os.path.join(model_path, '/mnt/storage/yxh/competition24/lightweight/exp_dir/v15e35/ckpt/model_step1010000.pth'), map_location=self.device
            )
            self.model.load_state_dict(checkpoint["network_state_dict"], strict=False)

            self.model.eval()
            network = fuse_model(self.model)
            qat_qconfig_and_prepare(network, 'tfqat', scene_mode=0)
            self.model = torch.quantization.convert(network, inplace=False)
