...
flags.DEFINE_integer("file_size", 5000, "sample cnt per file")
...
def same_num(arrlist1, arrlist2):
    return [np.sum(np.all(np.equal(arr1, arr2), axis=1)) for arr1, arr2 in zip(arrlist1, arrlist2)]


def check(output_bin_0, output_bin_1, file_size):
    ...
    for out_0, out_1 in zip(loader_0, loader_1):
        ...
        top1_same = [x+y for x, y in zip(top1_same, same_num(top1_0, top1_1))]
        top2_same = [x+y for x, y in zip(top2_same, same_num(top2_0, top2_1))]

        if total >= file_size:
            file_cnt += 1
            top1_same = [top1_same[-1]] + top1_same[:-1]
            top2_same = [top2_same[-1]] + top2_same[:-1]
            
            top1_same_total = [x+y for x, y in zip(top1_same_total, top1_same)]
            top2_same_total = [x+y for x, y in zip(top2_same_total, top2_same)]
            
            print('File %d\tTop1\t%.2f%%(%.2f%%,%.2f%%,%.2f%%,%.2f%%,%.2f%%)\tTop2\t%.2f%%(%.2f%%,%.2f%%,%.2f%%,%.2f%%,%.2f%%)' % (file_cnt, *(x / total * 100 for x in top1_same), *(x / total * 100 for x in top2_same)))
            total = 0
            top1_same = [0] * 6
            top2_same = [0] * 6
    total = file_cnt * file_size
    print('Total\tTop1\t%.2f%%(%.2f%%,%.2f%%,%.2f%%,%.2f%%,%.2f%%)\tTop2\t%.2f%%(%.2f%%,%.2f%%,%.2f%%,%.2f%%,%.2f%%)' % (*(x / total * 100 for x in top1_same_total), *(x / total * 100 for x in top2_same_total)))
    print(f"{top1_same_total[0] / total}\t{top2_same_total[0] / total}")
    return True


def main(_):
    FLAGS = flags.FLAGS
    ret = check(FLAGS.output_bin_0, FLAGS.output_bin_1, FLAGS.file_size)
    if not ret:
        print("Not equal")
        exit(-1)
