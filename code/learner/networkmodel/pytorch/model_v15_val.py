...
class NetworkModel(nn.Module):
    ...
    def compute_loss(self, data_list, rst_list):
        ...
        for hero_index in range(len(data_list)):
            ...
            masked_equal_list = [(x == y) & mask for x, y, mask in zip(pred_action_list, this_hero_action_list, sub_action_mask_list)]
            ...
            acc_list = [(torch.sum(x) / torch.sum(y)) for x, y in zip(masked_equal_list, sub_action_mask_list)]
            all_acc_list.append(acc_list)

        # all_hero_loss_list (3, 5)   all_acc_list (3, 5)
        return cost_all, all_hero_loss_list, all_acc_list
