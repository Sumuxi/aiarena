...
class NetworkModel(nn.Module):
    ...
    def fuse_model(self):
        torch.ao.quantization.fuse_modules(self.conv_layers, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.hero_share_mlp, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.public_info_mlp, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.soldier_share_mlp, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.organ_share_mlp, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.monster_mlp, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.concat_mlp, ['0', '1'], inplace=True)
        torch.ao.quantization.fuse_modules(self.target_head, ['0', '1'], inplace=True)
        for m in self.action_heads:
    ...
    def forward(self, data_list):
        ...
        # 循环展开
        hero_data = data_list[0]
        hero_feature = hero_data[0]
        # 量化
        hero_feature = self.quant(hero_feature)
        
        feature_img, hero_frd, hero_emy, public_info, soldier_frd, soldier_emy, organ_frd, organ_emy, monster_vec, global_info = hero_feature.split([17 * 17 * 6, 251 * 3, 251 * 3, 44, 25 * 10, 25 * 10, 29 * 3, 29 * 3, 28 * 20, 68], dim = 1)
        ...
        hero_target_list.insert(0, torch.ones_like(hero_target_list[2]) * 0.1)
        all_hero_target_list.append(torch.cat(hero_target_list, dim = 1))
        
        concat_hidden = torch.cat([conv_hidden, hero_frd_hidden_pool, hero_emy_hidden_pool, public_info_hidden, soldier_frd_hidden_pool, soldier_emy_hidden_pool, organ_frd_hidden_pool, organ_emy_hidden_pool, monster_hidden_pool, global_info], dim = 1) # 192+64+64+32+32+32+32+32+32+68=580
        ...
        # 反量化
        hero_result_list[0] = self.dequant(hero_result_list[0])
        hero_result_list[1] = self.dequant(hero_result_list[1])
        hero_result_list[2] = self.dequant(hero_result_list[2])
        hero_result_list[3] = self.dequant(hero_result_list[3])
        hero_result_list[4] = self.dequant(hero_result_list[4])
        hero_result_list[5] = self.dequant(hero_result_list[5])
