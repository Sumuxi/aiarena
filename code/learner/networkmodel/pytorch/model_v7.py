...
class NetworkModel(nn.Module):
    def __init__(self):
        ...
        self.hero_frd_mlp = nn.Linear(256, 64)
        self.hero_emy_mlp = nn.Linear(256, 64)
        ...
        self.soldier_frd_mlp = nn.Linear(64, 32)
        self.soldier_emy_mlp = nn.Linear(64, 32)
        ...
        self.organ_frd_mlp = nn.Linear(64, 32)
        self.organ_emy_mlp = nn.Linear(64, 32)
        ...
        self.align_decoder_mlp = nn.Sequential(
            nn.Linear(580, 2048),
            nn.ReLU(),
            nn.Linear(2048, 1024),
            nn.ReLU(),
        ...
        self.align_lstm_mlp = nn.Sequential(
            nn.Linear(1024, 512),
            nn.ReLU(),
            nn.Linear(512, 512),
            nn.ReLU(),
        ...
        # self.projection_align1 = nn.Linear(256, 1024)  # for encoder out
        self.target_embed = nn.Linear(32, 32)
    ...
    def forward(self, data_list):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            conv_hidden = self.conv_layers(feature_img).flatten(start_dim=1)  # B*12*4*4, B*192
            # 改成reshape
            ...
            hero_target_list.append(organ_emy_hidden_pool.reshape((-1, 1, 32)))  # 1 emy organ
            ...
            concat_all_feature = torch.cat(
                [conv_hidden, hero_frd_hidden_pool, hero_emy_hidden_pool, public_info_hidden, soldier_frd_hidden_pool,
                 soldier_emy_hidden_pool, organ_frd_hidden_pool, organ_emy_hidden_pool, monster_hidden_pool,
                 global_info], dim=1)  # 192+64+64+32+32+32+32+32+32+68=580
            align_decoder_hidden = self.align_decoder_mlp(concat_all_feature)
            all_hero_align_decoder_list.append(align_decoder_hidden)
            align_lstm_hidden = self.align_lstm_mlp(align_decoder_hidden)

            # align_encoder_out = self.projection_align1(concat_hidden)

            concat_hidden_split = align_lstm_hidden.split((64, 192), dim=1)
        ...
        for hero_index in range(self.hero_num):
            ...
            target_key = self.target_head(fc_public_result).reshape((-1, 32, 1))  # B*32*1
        ...
        return all_hero_result_list, all_hero_align_decoder_list
    ...
    def _calculate_single_hero_distill_loss(self, unsqueeze_label_list, student_logits_list, teacher_logits_list,
                                            unsqueeze_weight_list, align_layers, teacher_align_layers,
                                            temperature=4.0, lambda_weight=0.5):
        label_list = []
        for ele in unsqueeze_label_list:
            label_list.append(torch.squeeze(ele, dim=1).long())
        weight_list = []
        ...
        for i in range(len(label_list)):
            ...
            layer_loss = F.mse_loss(torch.cat(align_layers, dim=0), torch.cat(teacher_align_layers, dim=0), reduction='mean')
            ...
            print("loss: ", hard_label_final_loss.item(), soft_label_final_loss.item(), layer_loss.item())
    ...
    def compute_loss(self, data_list, rst_list, align_layers, teacher_rst_list, teacher_align_layers):
        ...
        for hero_index in range(len(data_list)):
            ...
            student_prob_list = [torch.softmax(x, dim=-1) for x in this_hero_fc_label_list]
            ...
            # teacher net output
            teacher_logits_list = teacher_rst_list[hero_index][:-1]
            teacher_prob_list = [torch.softmax(x, dim=-1) for x in teacher_logits_list]
            teacher_action_list = [torch.argmax(x, dim=-1, keepdim=True) for x in teacher_prob_list]

            # hard label loss
            this_hero_hard_loss_list = self._calculate_single_hero_hard_loss(teacher_action_list,
