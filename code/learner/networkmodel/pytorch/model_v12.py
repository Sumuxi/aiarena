...
class NetworkModel(nn.Module):
    def __init__(self):
        ...
        self.soldier_frd_mlp = nn.Linear(256, 64)
        self.soldier_emy_mlp = nn.Linear(256, 64)
        self.organ_share_mlp = nn.Sequential(
            nn.Linear(29, 256),
            nn.ReLU(),
        )
        self.organ_frd_mlp = nn.Linear(256, 64)
        self.organ_emy_mlp = nn.Linear(256, 64)
    ...
    def forward(self, data_list):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            hero_target_list.append(public_info_hidden.reshape((-1, 1, 64)))  # 1 public info
        ...
        for hero_index in range(self.hero_num):
            ...
            target_key = self.target_head(fc_public_result).reshape((-1, 64, 1))  # B*128*1
